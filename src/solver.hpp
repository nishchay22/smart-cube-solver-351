...
// --- 1. DEFINITIONS (WCA Standard) ---
const string MOVE_STR[] = {"U", "U2", "U'", "R", "R2", "R'", "F", "F2", "F'", "D", "D2", "D'", "L", "L2", "L'", "B", "B2", "B'"};

// Tables Sizes
const int N_CO = 2187;    // 3^7
const int N_EO = 2048;    // 2^11
const int N_SL = 495;     // C(12,4)
const int N_CP = 40320;   // 8!
const int N_EP8 = 40320;  // 8! (U/D Edges)
const int N_EP4 = 24;     // 4! (Slice Edges)
...
// --- 4. CORE MOVE LOGIC (Standard WCA Cycles) ---
// (Kept as global helper function since it's stateless)
...
// --- 6. GLOBAL TABLES (Read-Only after Init) ---
unsigned short move_co[N_CO][18];
unsigned short move_eo[N_EO][18];
unsigned short move_sl[N_SL][18];
unsigned short move_cp[N_CP][18];
unsigned short move_ep8[N_EP8][18];
unsigned short move_ep4[N_EP4][18];

int8_t prune_co[N_CO];
int8_t prune_eo[N_EO];
int8_t prune_sl[N_SL];
int8_t prune_cp[N_CP];
int8_t prune_ep[N_EP8];
int8_t prune_ep4[N_EP4];
...
void build_tables() {
    ...
    build_table<N_CO>(move_co, prune_co, get_co);
    build_table<N_EO>(move_eo, prune_eo, get_eo);
    build_table<N_SL>(move_sl, prune_sl, get_slice);
    build_table<N_CP>(move_cp, prune_cp, get_cp, true);
    build_table<N_EP8>(move_ep8, prune_ep, get_ep8, true);
    build_table<N_EP4>(move_ep4, prune_ep4, get_ep4, true);
...
class Solver {
private:
    // Instance-specific variables (Safe for concurrency)
    vector<string> moves;
    bool solved_found = false;
    string solution_str = "";

    // Helpers need to be member methods now to access 'moves'
    ...
    // Helper to parse scramble
    Cube execute_scramble(string s) {
        Cube c;
        stringstream ss(s);
        string seg;
        while(ss >> seg) {
...
public:
    // Main Entry Point
    string solve(string scramble) {
        // Clear state for this specific request
        moves.clear();
        solution_str = "";
        solved_found = false;
